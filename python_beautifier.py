class Beautifier:
    def __init__(self):
        ...
        # data will be stored here before it is concatenated to the output string.
        # dummy empty strings so that certain index calls don't return out of bounds
        self.data = ["",""]
        ...
        self.last_non_spacing_token_type = ""
        self.last_text = ""
        ...
        self.chars_in_prev_line = 0
        self.line_indent_level = -1
        self.index_of_last_paren_keyword = -1
        self.index_of_closing_paren_for_keyword = -1
        ...
        # find a way to set this to be the sublime configured num of spaces in a tab
        self.tab_spaces = 4
        self.line_limit = 80
        ...
        self.spacing_tokens = ["TK_NEW_LINE", "TK_CARRIAGE_RETURN", "TK_WHITE_SPACE"]
        ...
        self.keywords = ["and","assert","break","class","continue","def","del","elif","else",
            "except","exec","finally","for","from","global","if","import","in","is","lambda",
            "not","or","pass","print","raise","return","try","while","yield"]
        self.keywords_with_parens_and_colons = ["elif","if","while"]
        self.keywords_with_colons_and_no_parens = ["class","def","else","except","finally","try","for","lambda"]
        #self.keywords_with_braces = ["assert", "elif", "except", "exec", "for", "if", "lambda", 
    ...
    def beautify(self, input):
        ...
        while (True):
            ...
            handlers = {
                'TK_STRING_LITERAL': self.handle_string_literal,
                'TK_OPERATOR': self.handle_operator,
                'TK_OTHER': self.handle_other,
                'TK_NEW_LINE': self.handle_new_line,
                'TK_CARRIAGE_RETURN': self.handle_carriage_return,
                'TK_WHITE_SPACE': self.handle_white_space,
                'TK_OPEN_PAREN': self.handle_open_paren,
                'TK_CLOSE_PAREN': self.handle_close_paren,
                'TK_COMMENT': self.handle_comment,
                'TK_PUNCTUATION': self.handle_punctuation,
                'TK_KEYWORD': self.handle_keyword,
                'TK_COLON': self.handle_colon
            }
    ...
    def get_next_token(self):
        ...
        if c in self.string_literal_characters:
            ...
            if (parser_pos + 1 < len(self.input) and self.input[parser_pos] == self.input[parser_pos+1] == c):
                triple_quotes = True
                sep = sep*3
                parser_pos += 2
            ...
            if parser_pos < \
                ...
                if triple_quotes == False:
                    ...
                    cond = (parser_pos + 2 < len(self.input) and self.input[parser_pos:parser_pos+3] != sep) or esc
                while cond:
            ...
            operator = c
            c = self.input[parser_pos]
            while parser_pos < len(self.input) and (operator + c)  in self.operators:
                operator += c
                parser_pos += 1
                c = self.input[parser_pos]
            ...
            if (parser_pos < len(self.input) and 
                self.input[parser_pos] not in self.spacing_characters):
                resulting_comment += " "
            while parser_pos < len(self.input) and self.input[parser_pos] not in self.newline_characters:
        ...
        # handle the TK_OTHER case
            ...
            while parser_pos < len(self.input):
                c = self.input[parser_pos]
                if (c not in ['"', "'", "!", " ","\t","\r","\n","(","[","{",")","]","}","#",":"] and 
                    c not in self.operators and c not in self.punctuation):
                    resulting_string += c
                    parser_pos += 1
                else:
                    break
    ...
    def handle_operator(self, token_text):
        if token_text in self.operators_maybe_pre_space:
            if self.last_last_token_type == "TK_OPEN_PAREN" and self.last_token_type == "TK_WHITE_SPACE":
                self.pop()
    ...
    def handle_other(self, token_text):
        # The following if statement handles removing the mandatory space inserted following
        ...
        # - OTHER
        if self.data[len(self.data)-2] in self.operators_unary and self.last_token_type == "TK_WHITE_SPACE": 
            # (- OTHER
    ...
    def handle_keyword(self,token_text):
        ...
        if token_text in self.keywords_with_colons_and_no_parens:
            self.index_of_last_paren_keyword = -1
        elif self.add_parens_to_keywords_with_colons == True and token_text in self.keywords_with_parens_and_colons and self.paren_nested_level == 0:
            self.index_of_last_paren_keyword = len(self.data)
    ...
    def handle_white_space(self, token_text):
        if self.last_token_type in ["TK_NEW_LINE", "TK_CARRIAGE_RETURN"]:
            ...
            pass
        elif (self.last_token_type == "TK_COLON" and 
              self.paren_type[self.paren_nested_level] == "["):
            pass
        else:
            self.append_token(token_text, "TK_WHITE_SPACE")

    def handle_new_line(self, token_text):
        # we let the line wrapping algorithm handle new lines if we are inside any type of parens. Hence, we ignore new line symbols we read in here
        if(self.paren_nested_level > 0 and token_text != "\nFORCE" and self.last_token_type != "TK_COMMENT"):
            return
        if token_text == "\nFORCE":
            token_text = "\n"
    ...
    def handle_open_paren(self, token_text): 
        if self.last_last_token_type not in ["TK_OPERATOR","TK_KEYWORD"] and self.last_token_type == "TK_WHITE_SPACE":
            self.pop()
        if(self.last_token_type in ["TK_KEYWORD"]):
            self.append_token(" ","TK_WHITE_SPACE")
       
    #    print "handling open paren, keyword was: %s" %self.data[len(self.data)-2]
    #    print "paren nested level is: %r" % self.paren_nested_level
    #    if(self.add_parens_to_keywords_with_colons == True and 
    #       self.data[len(self.data)-2] in self.keywords_with_parens_and_colons and 
    #       self.paren_nested_level == 1):
    #        self.index_of_closing_paren_for_keyword = 0
    ...
    def handle_close_paren(self, token_text):
        if self.last_token_type == "TK_WHITE_SPACE" and self.last_last_token_type not in self.spacing_tokens:
            self.pop()

    #    print "paren nested level: %r" % self.paren_nested_level
    #    print "index of closing paren: %r" % self.index_of_closing_paren_for_keyword 
    #    print "data length: %r" % len(self.data)
    #    if(self.add_parens_to_keywords_with_colons == True and 
    #       self.paren_nested_level == 0 and
    #       self.index_of_closing_paren_for_keyword == 0):
    #        self.index_of_closing_paren_for_keyword = len(self.data)
    ...
    def handle_comment(self, token_text):
        ...
        # wrap long comments
        if self.chars_since_new_line + len(token_text) > self.line_limit and self.wrap_comments == True:
            token_text_list = list(token_text)
            new_line_range = range(0, self.line_limit - self.chars_since_new_line)
        ...
        # colon must have been used with a keyword, e.g. "else:"
            ...
            if(self.add_parens_to_keywords_with_colons == True):
                ...
                if(self.last_token_type != "TK_CLOSE_PAREN"):
                    ...
                    loop_range.reverse()
                    for i in loop_range:
                        if self.data[i] in self.keywords_with_colons_and_no_parens:
                            add_parens_here = False
                            break
                        elif self.data[i] in self.keywords_with_parens_and_colons:
                            break
                    ...
                    for i in loop_range:
                        if self.data[i] in self.keywords_with_colons_and_no_parens:
                            add_parens_here = False
                            break
                        elif self.data[i] in self.keywords_with_parens_and_colons and closing_paren_count != -1:
                            # we found a keyword before finding the matched open paren.
                            ...
                            if closing_paren_count == -1: 
                                ...
                                if self.data[i-2] in self.keywords_with_parens_and_colons:
                                    add_parens_here = False
                                    break
    ...
    def append_token(self, token_text, token_type):
        if(self.line_indent_level == -1 and token_type not in self.spacing_tokens):
            self.line_indent_level = self.chars_since_new_line

        # wrap lines if we are inside some sort of parens
        if(self.chars_since_new_line + len(token_text) > self.line_limit and self.paren_nested_level > 0 and self.wrap_inside_parens):
            # wrap only on these tokens
            if(token_type in ["TK_STRING_LITERAL","TK_OTHER","TK_KEYWORD"]):
                self.handle_new_line("\nFORCE")
                # We wrap to align with the outter most paren block. If we were instead to wrap to self.open_paren_index[self.paren_nested_level] it would create ugly issues in some lines
        ...
        if token_text == "\t":
            ...
            self.chars_in_prev_line = self.chars_since_new_line
            self.chars_since_new_line = 0
            self.line_indent_level = -1
        else:    
